...
class MCSLayer(LayerAutomata):
    ...
    is why it can do everything and more!
    """
    ...
    def allChannelConnected(self):
        ...
        for channelId in self._channels:
            layer = self._channels[channelId]
            log.debug("Connecting channel {} ({})".format(channelId, type(layer)))
            #use proxy for each channel
            proxy = MCSLayer.MCSProxySender(layer, self, channelId)
            proxy.connect()
...
class Client(MCSLayer):
    ...
    def connectNextChannel(self):
        ...
        if self._nbChannelRequested < self._serverSettings.getBlock(gcc.MessageType.SC_NET).channelCount.value:
            ...
            self.sendChannelJoinRequest(channelId.value)
            return
        ...
        if confirm == 0:
            ...
            for i in range(0, serverNet.channelCount.value):
                if channelId == serverNet.channelIdArray[i].value:
                    ...
                    log.debug("VirtualChannel {} ({}) confirmed".format(channelId, type(self._channels[channelId]).__name__))
